...
app, rt, rooms, Room = fast_app(
    "data/drawapp.db", render=render, id=int, name=str, created_at=str, pk="id"
)
...
def get():
    create_room = Form(
        Input(id="name", name="name", placeholder="New Room Name"),
        Button("Create Room"),
        hx_post="/rooms",
        hx_target="#rooms-list",
        hx_swap="afterbegin",
    )
